...
for n in range(1, HI, 2):
    for m in range(1, WI, 2):
        rectunitrow = numpy.append(rectunitrow, (intersecting_coordinate[n-1][m-1], intersecting_coordinate[n-1][m], \
                                                 intersecting_coordinate[n][m-1], intersecting_coordinate[n][m]))    # 在单元格矩阵中每行为一个单元格各顶点的坐标add coordinates of the four vettexes of the each cell in the table to the matrix in each row
